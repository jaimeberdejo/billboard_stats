...
export function ArtistDrilldown({ payload }: ArtistDrilldownProps) {
  ...
  if (payload.rows.length === 0) {
    if (payload.weeks && payload.weeks.length > 0) {
      return (
        <div className="divide-y divide-black/10 bg-[#FAFAFA]">
          {payload.weeks.map((week) => (
            <section key={week.chart_date} className="px-4 py-4">
              <div className="flex items-end justify-between gap-3">
                <div>
                  <div className="text-[12px] font-[700] leading-[1.3] text-[#0A0A0A]">
                    {week.chart_date}
                  </div>
                  <div className="mt-0.5 text-[10px] font-[600] uppercase tracking-[0.08em] text-[#888888]">
                    {week.rows.length} entries
                  </div>
                </div>
              ...
              <div className="mt-3 overflow-hidden rounded border border-black/10 bg-white">
                <div className="divide-y divide-black/10">
                  {week.rows.map((row) => {
                    ...
                    return (
                      <button
                        key={`${week.chart_date}-${href}-${row.value}`}
                        type="button"
