...
def _get_latest_chart_dates(conn) -> dict:
    """Query DB for the latest chart_date per chart_type."""
...
def update_charts(conn, data_dir: str = None):
    ...
    if b200_latest:
        start_year = b200_latest.year
        end_year = today.year
        logger.info(f"Billboard 200: fetching years {start_year}-{end_year}")
        download_b200(start_year, end_year, data_dir)
...
def repair_gaps(conn, data_dir: str = None, since_year: int = 2025):
    ...
    for chart_type in ["hot-100", "billboard-200"]:
        missing = find_failed_downloads(chart_type, since_year, current_year, data_dir)
        # Filter out future dates
        missing = [d for d in missing if d <= today.isoformat()]
        ...
        if chart_type == "hot-100":
            ...
            years = sorted(set(int(d[:4]) for d in missing))
            download_b200(years[0], years[-1], data_dir, overwrite=True)
            b200_dir = os.path.join(data_dir, "b200")
            loaded = [d for d in missing
